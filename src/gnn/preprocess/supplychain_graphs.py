...
import sys
sys.path.append('/data/yanjia/MAS_SupplyChain/src/gnn/preprocess')
from generate_split import generate_split
from lm_modeling import load_model, load_text2embedding
from .utils.retrieval import retrieve_subgraph, get_sub_df_nodes, get_sub_df_edges
...
parser.add_argument('--kaping', action='store_true')
...
def generate_text_embedding(require_retrieval: bool = True):
    ...
    def _encode_graph():
        ...
        for i in tqdm(data_list):
            if require_retrieval:
                ...
                nodes = pd.read_csv(f'{path_nodes}/{i}.csv') # the agents should not know others private information, set by default
...
def preprocess(filename: str, require_retrieval: bool):
    ...
    for index in tqdm(range(len(questions))):
        ...
        df_nodes = pd.read_csv(f'{path_original_nodes}/{data_idx}.csv')
        df_edges = pd.read_csv(f'{path_original_edges}/{data_idx}.csv')
        ...
        if require_retrieval:
            ...
            df_simp_edges = get_sub_df_edges(df_nodes=df_nodes, df_edges=df_edges, G=G, target_node=target_node)
            df_sub_nodes.to_csv(f'{path_nodes}/{data_idx}.csv', index=False)
            df_simp_edges.to_csv(f'{path_edges}/{data_idx}.csv', index=False)

            desc = df_sub_nodes[['node_id', 'node_attr']].to_csv(index=False)+'\n'+ df_simp_edges[['src', 'edge_attr', 'dst']].to_csv(index=False)
...
def preprocess_kaping(filename: str):
    ...
    for index in tqdm(range(len(questions))):
        ...
        # df_nodes, df_edges = retrieve_subgraph(df_nodes=df_nodes, df_edges=df_edges, target_node=target_node, env=env, G=G, data_type=filename)
        df_nodes = pd.read_csv(f'{cached_nodes}/{data_idx}.csv')
        df_edges = pd.read_csv(f'{cached_edges}/{data_idx}.csv')

        desc = []
        for i in range(len(df_nodes)):
            if len(df_nodes.iloc[i]['node_attr'].split(",")) > 0:
                desc.append(f"(id {df_nodes.loc[i, 'node_id']}, {df_nodes.loc[i, 'node_attr']})")
        for i in range(len(df_edges)):
            desc.append(f"(id {df_edges.loc[i, 'src']}, {df_edges.loc[i, 'edge_attr']}, id {df_edges.loc[i, 'dst']})")
...
if __name__ == '__main__':
    ...
    path_original_nodes = f'{PATH}/{data_type}_data/original_nodes'
    path_original_edges = f'{PATH}/{data_type}_data/original_edges'
    ...
    def parallel_kaping_preprocess(filename):
        preprocess_kaping(filename=filename)
    ...
    if not kaping:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            executor.map(parallel_kaping_preprocess, filenames)
