...
env_configs_list = {
    "graph_4_4": {
        ...
        "max_num_agents_per_stage": 6, # can add new agents in the middle way
        "num_periods": 30,
...
def get_env_configs(env_configs: dict):
    ...
    num_total_agents = num_stages * max_num_agents_per_stage
    ...
    order_costs, sale_prices, prod_costs = \
        generate_cost_price(prod_cost_dist=env_configs["price_cost_dist"], profit_rate_dist=env_configs["profit_rate_dist"], \
                            num_stages=num_stages, num_agents_per_stage=max_num_agents_per_stage, config_name=env_configs["config_name"])
    holding_costs = \
    ...
    llm_controlled_agents = env_configs["llm_agents"]

    llm_agents = np.zeros((num_stages, max_num_agents_per_stage), dtype=int)
    for i, j in llm_controlled_agents:
        llm_agents[i, j] = 1
    ...
    return {
        ...
        'init_inventories': init_inventories, # num_stages * max_num_agents_per_stage
        'lead_times': lead_times, # num_stages * max_num_agents_per_stage * max_num_agents_per_stage
        'demand_fn': demand_fn,
