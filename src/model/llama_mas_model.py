...
def create_agents(stage_name: List[str], num_agents_per_stage: int, args) -> List[AutoModelForCausalLM]:
    
    agents = []
    num_stages = len(stage_name)

    # Build Model
    llm_model_path = llama_model_path[args.llama_model_name]
    model = load_model[args.model_name](args=args, graph=None, graph_type=None)
    agents.append(model)
...
#                name=f"MAS_{args.dataset}_{args.model_name}_seed{seed}",
#                config=args)
...
#     print(args)
    ...
    #     generated_text = f"Generated text for epoch {epoch}"
