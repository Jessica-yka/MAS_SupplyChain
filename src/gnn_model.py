## write a GCN based model for graph representation learning
import dgl
import dgl.function as fn
import torch as th
import torch.nn as nn
import torch.nn.functional as F
from dgl import DGLGraph
import numpy as np
from sklearn.model_selection import train_test_split
import random
from sc_graph import create_agent_profiles
from config import env_configs, get_env_configs
import os
from transformers import BertModel, BertTokenizer
from transformers import LlamaForSequenceClassification, LlamaTokenizer
from torch.utils.data import Dataset, DataLoader

gcn_msg = fn.copy_u(u='h', out='m')
gcn_reduce = fn.sum(msg='m', out='h')


class GCNLayer(nn.Module):
    def __init__(self, in_feats, out_feats, edge_feats_dim):
        super(GCNLayer, self).__init__()
        self.linear = nn.Linear(in_feats + edge_feats_dim, out_feats)

    def forward(self, g, node_features, edge_features):
        with g.local_scope():
            g.ndata['h'] = node_features
            g.edata['e'] = edge_features
            g.update_all(self.message_func, self.reduce_func)
            h = g.ndata['h']
            return self.linear(h)

    def message_func(self, edges):
        return {'m': th.cat([edges.src['h'], edges.data['e']], dim=1)}

    def reduce_func(self, nodes):
        return {'h': th.sum(nodes.mailbox['m'], dim=1)}


class TextEncoder(nn.Module):
    def __init__(self, bert_model_name='bert-base-uncased'):
        super(TextEncoder, self).__init__()
        self.bert = BertModel.from_pretrained(bert_model_name)
        self.tokenizer = BertTokenizer.from_pretrained(bert_model_name)

    def forward(self, text):
        inputs = self.tokenizer(text, return_tensors='pt', padding=True, truncation=True, max_length=512)
        outputs = self.bert(**inputs)
        return outputs.last_hidden_state[:, 0, :]  # Use the [CLS] token representation



class Net(nn.Module):

    def __init__(self, node_in_feats, edge_in_feats, hidden_feats, out_feats):
        super(Net, self).__init__()
        self.layer1 = GCNLayer(node_in_feats, hidden_feats, edge_in_feats)
        self.layer2 = GCNLayer(hidden_feats, out_feats, edge_in_feats)


    def forward(self, g, node_features, edge_features):
        x = F.relu(self.layer1(g, node_features, edge_features))
        x = self.layer2(g, x, edge_features)
        return x


class GraphDataset(Dataset):
    def __init__(self, graphs, features, labels):
        self.graphs = graphs
        self.features = features
        self.labels = labels

    def __len__(self):
        return len(self.graphs)

    def __getitem__(self, idx):
        return self.graphs[idx], self.features[idx], self.labels[idx]

def collate_fn(batch):
    graphs, features, labels = zip(*batch)
    batched_graph = dgl.batch(graphs)
    batched_features = th.cat(features)
    batched_labels = th.cat(labels)
    return batched_graph, batched_features, batched_labels

def create_data_loader(graphs, features, labels, batch_size=32, shuffle=True):
    dataset = GraphDataset(graphs, features, labels)
    return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_fn)


# def load_sc_graph_data(agent_profiles, num_stage: int, num_agent_per_stage: int):
    
#     node_feats = []
#     edge_feats = []
#     g = dgl.DGLGraph()
#     node_labels = []
#     edge_u, edge_v = [], []
#     ndata = len(agent_profiles)
#     for ap in agent_profiles:
#         node_labels.append(ap.name)
#         node_features = ap.get_node_features()
#         node_feats.append(th.tensor(node_features, dtype=th.float32))  # Ensure features are float type
        
#         stage = ap.stage
#         agent = ap.agent
#         if stage < num_stage - 1:            
#             for dem, label in enumerate(ap.suppliers):
#                 if label == 1:
#                     edge_u.append(stage*num_agent_per_stage+agent)
#                     edge_v.append((stage-1)*num_agent_per_stage+dem)

#     g.add_nodes(ndata)
#     g.add_edges(edge_u, edge_v)
#     g.ndata['feat'] = th.tensor(node_feats)
#     return g, g.ndata['feat']







class LlamaTextClassifier(nn.Module):
    def __init__(self, llama_model_name='llama-base'):
        super(LlamaTextClassifier, self).__init__()
        self.llama = LlamaForSequenceClassification.from_pretrained(llama_model_name)
        self.tokenizer = LlamaTokenizer.from_pretrained(llama_model_name)

    def forward(self, text, gnn_embedding):
        inputs = self.tokenizer(text, return_tensors='pt', padding=True, truncation=True, max_length=512)
        outputs = self.llama(**inputs, labels=gnn_embedding)
        return outputs.logits

def train_llama_with_gnn_output(g, features, text_prompts, epochs=50, lr=1e-4):
    gnn_model = Net()
    llama_model = LlamaTextClassifier()
    optimizer = th.optim.Adam(list(gnn_model.parameters()) + list(llama_model.parameters()), lr=lr)
    
    for epoch in range(epochs):
        gnn_model.train()
        llama_model.train()
        
        gnn_embedding = gnn_model(g, features)
        text_prompt = random.choice(text_prompts)
        
        logits = llama_model(text_prompt, gnn_embedding)
        loss = F.cross_entropy(logits, gnn_embedding)
        
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        
        print(f"Epoch {epoch:05d} | Loss {loss.item():.4f}")

# Example text prompts
text_prompts = ["Supply chain optimization", "Inventory management", "Demand forecasting"]

train_llama_with_gnn_output(g, features, text_prompts)



if __name__ == "__main__":



    env_config_name = "large_graph_test"
    os.makedirs(f"env/{env_config_name}", exist_ok=True)
    env_config = get_env_configs(env_configs=env_configs[env_config_name])
    print(env_config.keys())
    agent_profiles = create_agent_profiles(env_config)

    # Example usage with Cora dataset
    g, features = load_sc_graph_data(agent_profiles=agent_profiles, num_stage=env_config['num_stages'], num_agent_per_stage=env_config['num_agents_per_stage'])
    print("edges", g.edges())
    u, v = g.edges()
    eids = np.arange(g.number_of_edges())
    eids = np.random.permutation(eids)
    test_size = int(len(eids) * 0.1)
    train_size = g.number_of_edges() - test_size
    train_pos_u, train_pos_v = u[eids[:train_size]], v[eids[:train_size]]
    test_pos_u, test_pos_v = u[eids[train_size:]], v[eids[train_size:]]

    train_neg_u = np.random.choice(g.number_of_nodes(), train_size)
    train_neg_v = np.random.choice(g.number_of_nodes(), train_size)
    test_neg_u = np.random.choice(g.number_of_nodes(), test_size)
    test_neg_v = np.random.choice(g.number_of_nodes(), test_size)

    train_pos_edges = (train_pos_u, train_pos_v)
    train_neg_edges = (train_neg_u, train_neg_v)
    test_pos_edges = (test_pos_u, test_pos_v)
    test_neg_edges = (test_neg_u, test_neg_v)

    train_link_predictor(g, features, train_pos_edges, train_neg_edges)